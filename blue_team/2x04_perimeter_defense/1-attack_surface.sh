...
for ((i = 0; i < SOCKET_COUNT; i++)); do
    ...
    local_addr=$(jq -r ".listening_sockets[$i].local_addr // .listening_sockets[$i].local_address // \"\"" "$BASELINE_FILE")
    local_port=$(jq -r ".listening_sockets[$i].local_port // 0" "$BASELINE_FILE")
    ...
    # ---------------------------------------------------------------
    # Look up function from service_catalog.json (port first, then process)
    # ---------------------------------------------------------------
    function_label=$(jq -r --arg port "$local_port" '.ports[$port] // ""' "$CATALOG_FILE")
    if [[ -z "$function_label" ]]; then
        function_label=$(jq -r --arg proc "$process" '.processes[$proc] // "unknown"' "$CATALOG_FILE")
    fi

    # ---------------------------------------------------------------
    # Look up criticality from service_criticality.json
    # ---------------------------------------------------------------
    criticality=$(jq -r --arg func "$function_label" '.[$func] // "medium"' "$CRIT_FILE")
